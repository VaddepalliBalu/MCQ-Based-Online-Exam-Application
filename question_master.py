...
# -----------------------------
# Read all questions as list, sanitizing invalid/empty rows.
def read_questions():
    try:
        with open(CSV_FILE, newline="", encoding="utf-8") as f:
            ...
            for row in reader:
                # Ensure all expected fields exist and strip whitespace
                cleaned = {k: (row.get(k, "") or "").strip() for k in FIELDS}

                # Normalize correctoption (allow '1'...'4' as well)
                norm_corr = normalize_correct_option(cleaned.get("correctoption", ""))
                cleaned["correctoption"] = norm_corr or ""

                # If any required field is empty or correctoption invalid, skip row
                if (
            ...
            if invalid_count > 0:
                # Renumber remaining questions sequentially and rewrite sanitized file
...
# -----------------------------
# Load questions as nested dictionary
...
# -----------------------------
# Add question (with strict validation)
def add_question():
    try:
        questions = read_questions()
        next_num = max([int(q["num"]) for q in questions], default=0) + 1

        # Get valid question text
        while True:
        ...
        # Get valid correct answer (allow numeric or opN)
        while True:
...
# -----------------------------
# Modify question (with validation; leave blank to keep old value)
def modify_question():
    try:
